...
impl Compilation {
    ...
    fn assign_module_ids(&mut self) {
        assign_module_ids(&self.options, &self.module_graph, &mut self.chunk_graph);
    }

    fn assign_chunk_ids(&mut self) {
        assign_chunk_ids(&self.options, &self.module_graph, &mut self.chunk_graph);
    }
    ...
    pub fn code_generation(&mut self) {
        ...
        assert!(
            self.ids_assigned,
            "Module and Chunk IDs must be assigned before code generation"
        );
