...
<https://trace.nextjs.org/>. The prebuilt `turbopack-cli` release may also
include `trace-decoder/` wasm assets for tooling that wants to decode raw trace
rows without starting the local trace server.
